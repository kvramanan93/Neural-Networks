...
@dataclass
class ModelConfig:
    block_size: int = None  # Length of an input sequence
    vocab_size: int = None
    ''' Params to control size of models '''
    n_layer: int = 4
    n_embd: int = 64
    n_embd2: int = 64
    n_head: int = 4
...
'''(init when a class is initialized and call when an instance of that class is called)'''


class RNNCell(nn.Module):
    """
    Takes input at a current time step and hiddenstate at the previous time step and returns the
    resulting hidden state at the current timestep
    """
...
class Bigram(nn.Module):
    """Bigram Model NN is just a lookup table for logits for the next character
    ...
    def get_block_size(self):
        # This is a bigram model, Only one block will be the blocksize
        return 1
...
if __name__ == '__main__':
    ...
    # system/input/output
    parser.add_argument('--input-file', '-i', type=str, default='merged_names.txt',
